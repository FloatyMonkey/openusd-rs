...
impl Mesh<'_> {
	...
	pub fn get(stage: &usd::Stage, path: impl Into<sdf::Path>) -> Option<Mesh<'_>> {
		let prim = stage.prim_at_path(path.into());
		if prim.is_valid() && prim.type_name() == Token::new("Mesh") {
			Some(Mesh(usd::SchemaBase::new(prim)))
		} else {
			None
		}
	}
